...
const adminAuth = fs.readFileSync(path.join(root, "Eye", "dart-admin-auth.js"), "utf8");
...
assert(finance.includes("renderReturningChart(current, previous)"), "Returning-customer chart must compare authoritative current and previous server buckets.");
...
assert(finance.includes("marginApplicable") && finance.includes("Margin N/A"), "Refund-only periods must not show a misleading positive margin.");
...
assert(finance.includes("downloadAuthoritativeFinanceCsv"), "P&L and Cash Flow exports must be downloaded from the authoritative backend.");
...
assert(finance.includes("FINANCE_VIEW_PERMISSIONS"), "Finance UI must map each sensitive metric group to an explicit backend permission.");
assert(finance.includes("FINANCE_MANAGE_PERMISSIONS"), "Finance UI must map each writable Finance resource to a granular manage permission.");
assert(finance.includes("/api/v1/admin/finance/section/"), "Granular Finance users must fetch only permitted server summary sections.");
assert(finance.includes("summaryAllows"), "Finance UI must refuse to render fields absent from the permitted server summary.");
assert(finance.includes("financeCanManage"), "Finance mutation controls must be permission gated in the browser as a UX boundary.");
assert(finance.includes("/api/v1/admin/finance/export"), "Authoritative Finance summary exports must be generated by the backend.");
assert(finance.includes("financeCanExport"), "Finance export controls must respect finance.export.");
assert(finance.includes("Metric access not granted"), "Goal actuals must distinguish restricted metrics from zero progress.");
assert(adminAuth.includes("canHydrateFinanceDomains"), "Finance-only Staff accounts must be able to hydrate only their permitted Finance domain rows.");
