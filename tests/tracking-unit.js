...
assert(representativeSource.includes('lastLocationSyncAt < LOCATION_SYNC_MS'), 'Active representative GPS sync must use the named one-second throttle');
assert(representativeSource.includes('LOCATION_SYNC_MS = 1000'), 'Representative GPS must be published at most once per second while active');
assert(representativeSource.includes('ROAD_ROUTE_REFRESH_MS = 6000'), 'Representative road route must recalculate every six seconds');
assert(trackingSource.includes('ROAD_ROUTE_REFRESH_MS = 6000'), 'Customer road route must recalculate every six seconds');
...
assert(!trackingSource.includes('state.hasRoadRoute = false;\n      state.route.setLatLngs([]);'), 'customer routing refresh must keep the last valid road route visible on provider failure');
