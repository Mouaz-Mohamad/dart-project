...
function setupModelModal() {
    ...
    if (costInput && sellingInput) {
        costInput.addEventListener('input', () => {
            ...
            if (cost > 0) {
                ...
                sellingInput.value = Math.round(suggestedSelling);
            } else {
                sellingInput.value = '';
            }
            calculateFinalPrice();
    ...
    function calculateFinalPrice() {
        if (!sellingInput || !discountInput || !finalPriceDisplay) return 0;
        const selling = parseFloat(sellingInput.value) || 0;
        const discount = parseFloat(discountInput.value) || 0;
        const finalPrice = selling - (selling * (discount / 100));
        finalPriceDisplay.textContent = `${Math.trunc(finalPrice)} EGP`;
        return finalPrice;
    }
    ...
    getSortedData(dataArray).forEach(item => {
        ...
        container.insertAdjacentHTML('beforeend', `
            <div class="${getRowClass(item)}" data-id="${item.id}">
                ...
                <span class="text-item w150 order-final-amount">${Math.trunc(finalAmount)} EGP</span>
...
function calculatePrices() {
    ...
    if (discountEl) discountEl.textContent = Math.trunc(totalDiscount) + ' EGP';
    if (totalEl) totalEl.textContent = Math.trunc(total) + ' EGP';
...
function dartMoney(v){ return `${Math.trunc(Number(v)||0)} EGP`; }
function dartEsc(v){
...
function dartAssignRepresentative(order,repId){ const rep=dartFindRepById(repId); if(!rep||dartIsArchived(rep)||rep.status!=='Active')return false; order.representativeId=rep.id; order.representativeBusinessId=rep.repId; order.representativeName=rep.name; order.representativePhone=rep.phone1||''; return true; }
...
function dartApplyTransition(order,target,meta={}){
    ...
    if(target==='Preparing' && prev==='Out With Representative'){ order.representativeId=null; order.representativeBusinessId=''; order.representativeName=''; order.representativePhone=''; }
    order.status=target;
