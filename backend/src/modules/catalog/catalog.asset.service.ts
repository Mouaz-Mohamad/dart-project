...
export class CatalogAssetService {
  ...
  async get(assetId: string): Promise<{ content: Buffer; contentType: string; originalName: string } | null> {
    const result = await this.pool.query<{
      content: Buffer;
      content_type: string;
      original_name: string;
    }>(
      "SELECT content, content_type, original_name FROM catalog_assets WHERE asset_id = $1",
      [assetId],
    );
    const row = result.rows[0];
    return row ? { content: row.content, contentType: row.content_type, originalName: row.original_name } : null;
  }
