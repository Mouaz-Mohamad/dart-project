// DART CODE GUIDE | backend/src/modules/identity/identity.service.ts
...
function customerPasswordPolicyOrThrow(password: string): void {
  ...
  if (problems.length > 0) {
    throw new AppError(422, "WEAK_PASSWORD", "Password must contain at least 6 characters", problems);
  }
}
...
export class IdentityService {
      ...
      await Promise.all([
        ...
        this.pool.query<{
          ...
          `SELECT s.user_id::text, s.staff_code, s.display_name,
                  ...
                  u.email_verified_at, u.last_login_at, s.disabled_at,
  ...
  public async login(
    ...
    if (accountType === "staff") {
      throw new AppError(
        403,
        "STAFF_EMAIL_ACCESS_REQUIRED",
        "Staff accounts use email verification instead of password login",
      );
    }
    ...
    try {
      ...
      const session = await this.issueSession(client, row.id, true, metadata);
