...
describe("reward database integrity migrations", () => {
  it("enforces one Birthday usage record per customer/year and finalizes only from order state", async () => {
    ...
    const timing = await migration("0043_reward_reservation_after_order_insert.sql");
  ...
  it("keeps draw history compatibility and serializes active-card decisions", async () => {
    const compatibility = await migration("0041_reward_history_reference_compatibility.sql");
    const lock = await migration("0042_dart_card_active_concurrency_lock.sql");
