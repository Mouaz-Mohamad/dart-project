...
const productionEnvironment = {
  ...
  OUTBOX_CRON_SECRET: "o".repeat(32),
};
...
describe("environment configuration", () => {
  it("loads safe defaults without exposing source values in errors", () => {
    ...
    expect(config.outboxCronSecret).toBeNull();
  });
  ...
  it("requires SMTP email delivery in production and validates its fields", () => {
    expect(() => loadConfig(productionEnvironment)).toThrow("EMAIL_PROVIDER");
    ...
    expect(smtp.outboxCronSecret).toBe("o".repeat(32));
  });
  ...
  it("validates optional direct WhatsApp settings", () => {
    ...
    expect(config.outboxCronSecret).toBe("o".repeat(32));
  });
});
