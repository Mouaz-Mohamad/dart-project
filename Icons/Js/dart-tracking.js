(function () {
  ...
  function currentOrder() {
    ...
    if (lastOrderId) {
      const last = orders.find(order => String(order.orderId) === String(lastOrderId) || String(order.id) === String(lastOrderId));
      if (last) return last;
    }
  ...
  function ensureMap() {
    if (map || typeof L === 'undefined' || !document.getElementById('tracking-map')) return map;
    map = L.map('tracking-map', {zoomControl:false, attributionControl:false}).setView([30.0444,31.2357],12);
  ...
  async function updateRoute(order, courier, destination) {
    ...
    lastRouteKey = key;
    try {
      const response = await fetch(`https://router.project-osrm.org/route/v1/driving/${courier.lng},${courier.lat};${destination.lng},${destination.lat}?overview=full&geometries=geojson`);
      const data = await response.json();
      const route = data.routes?.[0];
      if (!route) throw new Error('No route');
      const points = route.geometry.coordinates.map(([lng, lat]) => [lat, lng]);
      if (routeLine) routeLine.setLatLngs(points);
      else routeLine = L.polyline(points, {color:'#2563eb', weight:5, opacity:.9, lineCap:'round', lineJoin:'round'}).addTo(map);
      ...
      lastRouteKey = '';
      setSummary('The live location is visible, but route time is temporarily unavailable.');
      setEta('ETA: Updating');
    }
  }

  function updateMap(order) {
    const hasRepresentative = Boolean(order.representativeId || order.representativeBusinessId);
    if (!hasRepresentative) {
      setMapDisabled(true, 'Representative not assigned yet');
      setSummary('The delivery map will activate after a representative is assigned.');
    ...
    const courier = {lat:Number(order.courierLocation?.lat), lng:Number(order.courierLocation?.lng)};
    ...
    if (destinationMarker) destinationMarker.setLatLng([destination.lat,destination.lng]);
    else destinationMarker = L.marker([destination.lat,destination.lng], {icon:markerIcon('destination')}).addTo(currentMap).bindPopup('Your delivery address');

    if (hasCourier) {
      if (courierMarker) courierMarker.setLatLng([courier.lat,courier.lng]);
      else courierMarker = L.marker([courier.lat,courier.lng], {icon:markerIcon('courier')}).addTo(currentMap).bindPopup('Dart representative');
      currentMap.fitBounds([[destination.lat,destination.lng],[courier.lat,courier.lng]], {padding:[55,55],maxZoom:16});
      if (order.status === 'Delivered') {
        setSummary('This order has been delivered.');
        setEta('Delivered');
      } else {
        const age = Date.now() - Date.parse(order.courierLocation?.updatedAt || 0);
        if (age > 120000) {
      ...
      setSummary('Delivery address is fixed. Waiting for the representative to start delivery.');
      setEta('ETA: Not determined');
    }
  ...
  function renderProgress(order) {
    ...
    steps.forEach((step, stepIndex) => {
      ...
      if (circle) circle.textContent = stepIndex < index ? '✓' : String(stepIndex + 1);
    });
  ...
  function renderEmpty() {
    ...
    if (items) items.innerHTML = '<div class="empty-state">No order matches this tracking link.</div>';
    const driver = card.querySelector('.driver-box');
    if (driver) driver.innerHTML = '<div data-representative-empty>Representative not assigned yet</div>';
    document.querySelectorAll('.tracking-card .step').forEach(step => step.classList.remove('active'));
  ...
  function render() {
    ...
    const date = card.querySelector('.order-header div div:nth-child(2)');
    if (date) date.textContent = `Date: ${order.date || '-'} ${order.time || ''}`;
    ...
    const representativeAssigned = Boolean(order.representativeId || order.representativeBusinessId);
    ...
    if (driverName) driverName.textContent = representativeAssigned ? (order.representativeName || 'Dart representative') : 'Representative not assigned yet';
    if (driverMeta) driverMeta.textContent = representativeAssigned ? `Courier (ID: ${order.representativeBusinessId || order.representativeId})` : 'Waiting for assignment';
    const call = card.querySelector('.btn-call');
    if (call) {
      const visible = representativeAssigned && Boolean(order.representativePhone);
      call.hidden = !visible;
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (window.trackingMap) {
      try { window.trackingMap.remove(); } catch {}
      window.trackingMap = null;
    }
    render();
    setInterval(() => { if (!document.hidden) render(); }, 4000);
  });
  window.addEventListener('storage', event => { if (event.key === 'dart_orders') render(); });
})();
